...
type plainByteArrayColumnBuffer struct {
	plainByteArrayPage
	lengths []uint32
	scratch []byte
}

func newPlainByteArrayColumnBuffer(typ Type, columnIndex int16, numValues int32) *plainByteArrayColumnBuffer {
	return &plainByteArrayColumnBuffer{
		plainByteArrayPage: plainByteArrayPage{
			byteArrayPage: byteArrayPage{
				typ:         typ,
				values:      make([]byte, 0, typ.EstimateSize(int(numValues))),
				offsets:     make([]uint32, 0, numValues+1),
...
func (col *plainByteArrayColumnBuffer) Clone() ColumnBuffer {
	return &plainByteArrayColumnBuffer{
		plainByteArrayPage: plainByteArrayPage{
			byteArrayPage: byteArrayPage{
				typ:         col.typ,
...
func (col *plainByteArrayColumnBuffer) cloneLengths() []uint32 {
	lengths := make([]uint32, len(col.lengths))
	copy(lengths, col.lengths)
	return lengths
}

func (col *plainByteArrayColumnBuffer) ColumnIndex() (ColumnIndex, error) {
	return byteArrayColumnIndex{&col.byteArrayPage}, nil
}

func (col *plainByteArrayColumnBuffer) OffsetIndex() (OffsetIndex, error) {
	return byteArrayOffsetIndex{&col.byteArrayPage}, nil
}

func (col *plainByteArrayColumnBuffer) BloomFilter() BloomFilter { return nil }

func (col *plainByteArrayColumnBuffer) Dictionary() Dictionary { return nil }

func (col *plainByteArrayColumnBuffer) Pages() Pages { return onePage(col.Page()) }

func (col *plainByteArrayColumnBuffer) Page() Page {
	// The offsets have the total length as the last item. Since we are about to
	// expose the column buffer's internal state as a Page value we ensure that
	// the last offset is the total length of all values.
	col.offsets = append(col.offsets[:len(col.lengths)], uint32(len(col.values)))
	return &col.plainByteArrayPage
}

func (col *plainByteArrayColumnBuffer) Reset() {
	col.values = col.values[:0]
	col.offsets = col.offsets[:0]
	col.lengths = col.lengths[:0]
}

func (col *plainByteArrayColumnBuffer) NumRows() int64 { return int64(col.Len()) }

func (col *plainByteArrayColumnBuffer) NumValues() int64 { return int64(col.Len()) }

func (col *plainByteArrayColumnBuffer) Cap() int { return cap(col.lengths) }

func (col *plainByteArrayColumnBuffer) Len() int { return len(col.lengths) }

func (col *plainByteArrayColumnBuffer) Less(i, j int) bool {
	return bytes.Compare(col.index(i), col.index(j)) < 0
}

func (col *plainByteArrayColumnBuffer) Swap(i, j int) {
	col.offsets[i], col.offsets[j] = col.offsets[j], col.offsets[i]
	col.lengths[i], col.lengths[j] = col.lengths[j], col.lengths[i]
}

func (col *plainByteArrayColumnBuffer) Write(b []byte) (int, error) {
	_, n, err := col.writeByteArrays(b)
	return n, err
}

func (col *plainByteArrayColumnBuffer) WriteByteArrays(values []byte) (int, error) {
	n, _, err := col.writeByteArrays(values)
	return n, err
}

func (col *plainByteArrayColumnBuffer) writeByteArrays(values []byte) (count, bytes int, err error) {
	baseCount := len(col.lengths)
	baseBytes := len(col.values)

	col.appendPlainBytes(values)

	count = len(col.lengths) - baseCount
	bytes = len(col.values) - baseBytes
	return count, bytes, err
}

func (col *plainByteArrayColumnBuffer) WriteValues(values []Value) (int, error) {
	var model Value
	col.writeValues(makeArrayValue(values, unsafe.Offsetof(model.ptr)), columnLevels{})
	return len(values), nil
}
...
func (col *plainByteArrayColumnBuffer) index(i int) []byte {
	offset := col.offsets[i]
	length := col.lengths[i]
	end := offset + length
	return col.values[offset:end:end]
